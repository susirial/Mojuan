...
import useChatRolSendEmailForPswReset from '../hooks/useChatRolSendEmailForPswReset';
import { Form } from 'antd';  
import ReactSliderVerify from "react-slider-verify";
import "react-slider-verify/dist/index.css";
  
const ChatRolResetPass = () => {  
...
// 邮箱验证码状态
const { loading, errorMsg,handleEmailChange } = useChatRolSendEmailForPswReset(email);
...
interface ResetPasswdValues {
  password: string;
  email: string;
}
// 定义发送用户名和密码的 最终处理函数
const handleFinish = async (values: ResetPasswdValues): Promise<boolean | void>=> {  
  console.log('Received values of form: ', values);  
  setUseInfoUploading(true); // 开始提交时设置loading为true  
  try {  
      const response = await fetch('/register/chgpass', {  
      method: 'POST',  
      headers: {  
...
<Form.Item
  ...
  export default ChatRolResetPass;
