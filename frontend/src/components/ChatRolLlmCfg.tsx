...
const ToolDisplay = (props: {
  ...
  useEffect(() => {  
    // 如果 marked 是同步的，直接设置 HTML 字符串  
    // 如果 marked 是异步的（返回 Promise），则等待它解析  
    Promise.resolve(marked(tool.description))  
      .then((markup) => {  
        setDescriptionHtml(markup);  
      })  
      .catch((error) => {  
        // 在生产环境中，你可能想要更优雅地处理错误  
        console.error('Error processing markdown', error);  
        setDescriptionHtml(''); // 设置为空字符串或错误消息  
      });  
  }, [tool.description]); // 依赖数组，仅当 tool.description 变化时重新运行  
...
function ToolSelectionField(props: {
  ...
  const handleSelectTool = useCallback(
    (toolSchema: ToolSchema | null) => {
      ...
      if (toolSchema === null) {  
        // Handle the case when toolSchema is null  
        return;  
      }  
...
export function ChatRolLlmCfg(props: {
  className?: string;
  ...
  const readonly = !!props.config && !props.edit && !inflight;
   // console.log('susirial props ///',props)
      ...
      <div className="flex flex-row flex-1">
        <div className="relative flex flex-grow items-stretch focus-within:z-10">
          ...
          <Input
            ...
            id="key"
            autoComplete="off"
            className=" w-full rounded-none rounded-l-md border-0 py-1.5 pl-4 text-gray-900 ring-1 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 ring-inset ring-gray-300"
        ...
        <button
          type="submit"
          className="inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-r-md shadow-sm text-white bg-blue-500 hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600"
        >
          {inflight ? "保存中..." : "保存"}
        </button>
      </div>
